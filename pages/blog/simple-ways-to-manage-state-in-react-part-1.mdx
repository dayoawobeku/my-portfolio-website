...
and easier to maintain on the long run. </Text>}
...
the nearest common parent.</Text>}

<AnchorTag text="Prop Drilling" href="prop-drilling" />

{<Text>When building large applications, passing state can get a little
complicated when you have many layers of components and you need to pass some
piece of state deeply through the tree. The nearest common parent could be far
removed from the components that need data, and lifting state up that high can
lead to a situation sometimes called {''}

<QuoteText quote='prop drilling.' /></Text>}

{<Text>A simple way to look at prop drilling is that a parent creates a state,
passes it down to “middle men”; components that do not require the state, and
then down the tree eventually to the component(s) that needs it.</Text>}

{<Text>In other words, a component receives a state or props just so that
another component below it can access that. </Text>}

{<Text>Most react devs avoid it and would reach for libraries such as {''}

<QuoteText quote='redux' /> or <ExternalUrl text='React context' href='https://beta.reactjs.org/apis/react/useContext' /> the moment they encounter this problem.</Text>}
...
{<Text>Here, we have this Dashboard layout where the {''}

<QuoteText quote='WelcomeMessage' /> is being displayed based on the current user's name. To get the value, we would need to prop drill the  {''}
<QuoteText quote='user' /> object three levels deep until it gets to the <QuoteText quote='WelcomeMessage' /> component.</Text>}

{<Text>This is a contrived example, and prop drilling may not be a big deal in
such a scenario and in many instances for that matter, but it can be an
annoyance in really complex state management architectures.</Text>}

{<Text>When you truly encounter the latter and prop drilling may potentially
complicate your state management, then there's a couple of other solutions we
can use to avoid prop drilling. The first is {''}

<QuoteText quote='component composition' />.</Text>}

<AnchorTag text="Component Composition" href="component-composition" />

{<Text>With composition, you basically make a prop that is the react element
that you want to render in a specific place.</Text>}

{<Text>Let's use the same prop drilling example and see how we can use
composition instead.</Text>}
...
{<Text>We have eliminated one level of prop drilling. We no longer pass the {''}

<QuoteText quote="DashboardNav" />
and {''}
<QuoteText quote="DashboardContent" /> inside the Dashboard, instead we pass it
the special {''}

<QuoteText quote='children' />
prop and pass the <QuoteText quote='user' /> directly to the <QuoteText quote='DashboardContent' />.</Text>}

{<Text>Let's take it a step further and do the same thing for the Dashboard
content.</Text>}
...
{<Text>And like that the Dashboard is more composable and customizable. And
we're no longer prop drilling.</Text>}

{<Text>Our app works like before too.</Text>}

<ComponentCompositionWithChildren />

{<Text>Sometimes you may need multiple “holes” in a component. Another way React
composes is to make a component that is a wrapper for another component. In such
cases you may come up with your own convention instead of using the {''}

<QuoteText quote='children' /> prop.</Text>}
...
{<Text>What we have simply done is pass arbitrary values as props to the {''}

<QuoteText quote='ChatApp'/> component.</Text>}

{<Text>React elements like <QuoteText quote='Contacts'/> and {''}

<QuoteText quote='Chat'/> are just objects, so they can be passed as props like
any other data. This is a very common use case for composition.</Text>}

{<Text>The next solution is the {''}

<ExternalUrl text='React context API' href='https://beta.reactjs.org/apis/react/useContext' />.</Text>}

<AnchorTag text="React Context" href="react-context" />

{<Text> Context is useful when you have some value that you want to make
accessible to a portion of your React component tree, without passing that value
down as props through each level of components.</Text>}

{<Text>We'll use {''}

<ExternalUrl text='react context' href='https://beta.reactjs.org/apis/react/useContext' />
to simplify the state management of our previous Dashboard app.</Text>}

{<Text>What we simply need to do is create a context using the {''}

<QuoteText quote='createContext'/> API, and wrap our <QuoteText quote='App'/> component with the
context provider.</Text>}

{<Text>We also removed the user object from the {''}
...
<ExternalUrl text='destructure' href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment' />
the <QuoteText quote='currentUser' /> object using the useContext API and use the <QuoteText quote='user' /> directly inside
the <QuoteText quote='WelcomeMessage' /> component.</Text>}
...
{<Text>Essentially, with client/ui state, start with just react. Most apps
barely have global app state after server state is handled. If you truly still
need a global state in your app, try {''}

<ExternalUrl text="zustand" href="https://www.npmjs.com/package/zustand" /> or{' '}

<ExternalUrl text="jotai" href="https://jotai.org/" />.

And for anything more complex, try {''}

<ExternalUrl text="xstate" href="https://xstate.js.org/" />.

</Text>}

{<Text>This was meant to be one really long blog post, but I decided to break it
...
/>
where I talk about managing server
state.</Text>}
